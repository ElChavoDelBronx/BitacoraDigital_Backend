...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody AuthRequest authRequest) {
        try {
            ...
            ApiResponse errorResponse = new ApiResponse(e.getMessage(), HttpStatus.UNAUTHORIZED);
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorResponse);
        }
    }

    @PostMapping("/first-login")
    public ResponseEntity<?> completeFirstLogin(@RequestBody ChangePasswordRequest request) {
        try {
            authService.changeInitialPassword(request.getEmail(), request.getCode(), request.getNewPassword());
